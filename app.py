...
Built with Streamlit for easy team access\
"""\
\
...
\}\
\
st.set_page_config(page_title="TruDiagnostic SOW Generator", page_icon="\uc0\u55358 \u56812 ", layout="wide")\
\
st.markdown("""\
<style>\
...
def generate_sow_document(data):\
    ...
    if data['track'] == 'processing':\
        ...
        lab_services = ["Sample Receipt & Accessioning \'96 Intake, verification, and LIMS entry of biological samples.", "Quality Control (QC) \'96 Comprehensive assessment of DNA quality, quantity, and integrity prior to analysis.", "DNA Extraction & Quantification \'96 Extraction from approved sample types with concentration/purity normalization."]\
        if data['processing_type'] == 'epigenetic':\
            lab_services.append("Epigenetic Processing \'96 DNA methylation analysis using Array Manufacturer or Company-developed arrays.")\
        else:\
            lab_services.append("Genetic Processing \'96 SNP and CNV detection using Array Manufacturer or custom genotyping arrays.")\
        lab_services.append("Sample & Data Storage \'96 Secure retention of residual materials and associated data.")\
    ...
    p.add_run("If identified in Exhibit A, the Company shall provide the following services:")\
    bio_services = ["Data Processing \'96 Conversion of raw array output into normalized and quality-controlled data sets.", "Algorithmic Analysis \'96 Application of Company's proprietary algorithms.", "QC Reporting \'96 Documentation of batch- and sample-level QC outputs."]\
    if data['bioinformatic_services']:\
        if any('irb' in svc for svc in data['bioinformatic_services']):\
            bio_services.append("IRB Submission \'96 Coordination of ethics review activities as specified in Exhibit A.")\
        if 'publication_drafting' in data['bioinformatic_services']:\
            bio_services.append("Publication Drafting \'96 Manuscript preparation for peer-reviewed journals.")\
        if 'publication_submission' in data['bioinformatic_services']:\
            bio_services.append("Publication Submission \'96 Journal submission support and coordination.")\
        if 'interventional_trial' in data['bioinformatic_services']:\
            bio_services.append("Interventional Trial Analysis \'96 Statistical analysis of interventional study designs.")\
        if 'algorithm_creation' in data['bioinformatic_services']:\
            bio_services.append("Custom Algorithm Development \'96 Development of tailored computational models.")\
        if 'algorithm_validation' in data['bioinformatic_services']:\
            bio_services.append("Algorithm Validation \'96 Independent performance evaluation of algorithms.")\
    ...
    p.add_run("If identified in Exhibit A, the Company shall provide the following services:")\
    report_services = [f"Data Delivery \'96 Secure transmission of data (\{', '.join(data['data_delivery'])\})."]\
    if data['portal_access']:\
        report_services.append("Technology Integration \'96 Portal access and customized data delivery formats.")\
    report_services.append("Data Security \'96 Encryption, HIPAA compliance, and adherence to Company's technical safeguards.")\
...
\
def main():\
    st.markdown('<p class="main-header">\uc0\u55358 \u56812  TruDiagnostic SOW Generator</p>', unsafe_allow_html=True)\
    ...
    with st.sidebar:\
        ...
        for i, step_name in enumerate(steps, 1):\
            if i < st.session_state.step:\
                st.success(f"\uc0\u10003  \{step_name\}")\
            elif i == st.session_state.step:\
                st.info(f"\uc0\u8594  \{step_name\}")\
            else:\
                st.text(f"\uc0\u9675  \{step_name\}")\
    ...
    with col1:\
        if st.session_state.step == 1:\
            ...
            st.markdown("---")\
            if st.button("Next \uc0\u8594 ", type="primary", disabled=not partner_name):\
            ...
            track = st.radio("Select service type:", options=["processing", "report_only"], format_func=lambda x: "\uc0\u55358 \u56810  **Sample Processing** - We process samples and provide reports" if x == "processing" else "\u55357 \u56522  **Report Only** - Partner uploads .idat files, we provide reports only", label_visibility="collapsed")\
            st.markdown("---")\
            col_a, col_b = st.columns(2)\
            with col_a:\
                if st.button("\uc0\u8592  Back"):\
                    st.session_state.step = 1\
                    st.rerun()\
            with col_b:\
                if st.button("Next \uc0\u8594 ", type="primary"):\
            ...
            if st.session_state.track == 'processing':\
                ...
                processing_type = st.radio("What type of processing?", options=["epigenetic", "genomic"], format_func=lambda x: f"\uc0\u55358 \u56812  \{CONFIG['processing_types'][x]['name']\}")\
            ...
            st.subheader("\uc0\u55357 \u56523  SOW Summary")\
            ...
            with col_a:\
                if st.button("\uc0\u8592  Back"):\
                    ...
                    st.rerun()\
            with col_b:\
                if st.button("\uc0\u55357 \u56580  Generate SOW Document", type="primary", use_container_width=True):\
            ...
            if st.session_state.get('doc_ready'):\
                st.success("\uc0\u9989  Document generated successfully!")\
                filename = f"SOW_\{st.session_state.partner_name.replace(' ', '_')\}_\{datetime.now().strftime('%Y%m%d')\}.docx"\
                st.download_button(label="\uc0\u55357 \u56549  Download SOW Document", data=st.session_state.doc_buffer, file_name=filename, mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True)\
                if st.button("\uc0\u55357 \u56580  Start New SOW"):\
                    for key in list(st.session_state.keys()):\
                        del st.session_state[key]\
                    st.rerun()\
    with col2:\
        st.markdown("### \uc0\u55357 \u56481  Quick Tips")\
        if st.session_state.step == 1:\
